...
Reply ONLY with valid JSON:
{
  "claims": [
    ...
    { "claim": "in 1883",               "status": "SUPPORTED", "sourceExcerpt": "..." }
        ...
        <Callout type="tip">
          ...
          <code>src/application/responseChecks/strategies/faithfulness.ts</code>. An offline eval
          scorer would be a thin wrapper around it — no duplication needed.
        </Callout>
      </Card>
  ...
  return dot / (norm(a) * norm(b));
        ...
        <div className="mt-4">
          <Callout type="info">
            Answer relevance is part of the proposed offline eval — not yet implemented. It would
            reuse the existing <code>VoyageEmbeddingAdapter</code> for the two embeddings.
          </Callout>
        </div>
      </Card>
...
Reply ONLY with JSON: { "covered": true }`}
        ...
        <Callout type="warning">
          Part of the proposed offline eval (not yet implemented). It requires a labeled{" "}
          <code>expected_answer</code> — write reference answers by reading the source document,
          never by asking the LLM. An LLM-generated reference inherits the same blind spots as the
